...
The inbreeding coefficient is usually referred to as __F__ (as reported in `plink --het`). It is defined as `1 - O(f(Aa))/E(f(Aa))`. But as you can see in the output of `plink --het`, it used `O(HOM)`, 3rd column, and `E(HOM)`, 4th column. With the total number of loci in the 5th column, `N(NM)`, you can do the calculation yourself and it should be `(O(HOM) - E(HOM)) / (N(NM) - E(HOM))`. The higher the F, the more inbreed, or more homozygous than expected.

OK this works for biallellic locus. How about multiallellic ones?



As you can see, this is a coefficient for a locus. How about an individual? How do we summarized this info for thousands or millions of loci? 

We know that `plink` offers the `--het` option, which `computes observed and expected homozygous/heterozygous genotype counts for each sample, and reports method-of-moments F coefficient estimates (i.e. (1 - (<observed het. count> / <expected het. count>)))`.

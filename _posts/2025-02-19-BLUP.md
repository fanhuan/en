...
This is the equation that we need to understand: Y = Xβ + Zu + ε
...
β: Fixed-effects coefficients (p×1).
Z: Random-effects design matrix (n×q): q is the dimension of A/G matrix. Design matrix needs y/n for perticular levels/individuals.  
u: Random effects (q×1), with u ∼ N(0, G) or u∼N(0, Aσ2u). Note that q can be greater than n, and this is why we can predict for more individuals than we have records of.
ε: Residual errors (n×1), with ε∼N(0,R).

Now we need to solve for β and u.

Recall how we solve for ordinary linear models (fixed effects only) using ordinary least squares using (X'X)β = X'Y to estimate β. How do we estimate β and u at the same time?

# Henderson's Mixed Model Equations 

Henderson developed these equations to estimate the fixed and random effects simultaneously. The key idea is to set up a system that incorporates both the fixed effects and the variance components associated with the random effects.
